...
StuckBitsService::
StuckBitsService(const fhicl::ParameterSet& pset, art::ActivityRegistry&)
: m_pran(nullptr) {
  // Fetch the random engine.
#ifdef UseSeedService
  int seed = seedSvc->getSeed("StuckBitsService");
#else
  int seed = 1009;
#endif
...
int StuckBitsService::modify(Channel, AdcCountVector& adcvec) const {
...
DEFINE_ART_SERVICE(StuckBitsService)
