...
int check(T1 v1, T2 v2) {
  ...
  T2 conval = con.convert<T2>(v1);
  if ( conval != v2 ) {
    cout << endl;
    cout << "  " << v1 << " --> " << conval << " != " << v2 << endl;
    assert(false);
    return 1;
  }
  return 0;
...
int test_float_to_int(string lab) {
  ...
  cout << endl;
  cout << "Checking float-to-int for " << lab << " ";
...
#endif
  const unsigned int nval = 9;
  double vals[nval] = {-1.0, 0.0, 1.0, 0.49, 0.50, 0.51, -0.49, -0.50, -0.51};
  SignalTypeConverter con;
  for ( double val : vals )  cout << setw(8) << val << " -->" << setw(3) << con.floatToInt(val) << endl;
